...
export function Step1Objetivo() {
  ...
  const opcoes: { id: ProjectType; title: string; desc: string; icon: React.ReactNode }[] = [
    ...
    { id: "sistema", title: "Sistema Web", desc: "Plataforma personalizada, SaaS ou dashboards sob medida.", icon: <Code size={20} /> },
  ];
    ...
    <div className="space-y-5 max-w-xl mx-auto w-full text-left py-0">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {opcoes.map((opcao) => {
            const isSelected = data.project.projectType === opcao.id;
            return (
              <Card
                key={opcao.id}
                onClick={() => update({ project: { projectType: opcao.id } })}

...
import { Code, CheckSquare, Calendar, FileText, User } from "lucide-react";
import { useWizard } from "../context/WizardContext";
import { ProjectType } from "@/features/projects/types";
import { BUDGET_ESTIMATE_MAP, DELIVERY_ESTIMATE_MAP, FEATURE_MAP, PROJECT_TYPE_MAP } from "@/features/projects/constants";

export function Step6Resumo() {
  const { data } = useWizard();
const typeKey = data.project.projectType as ProjectType;
const projetoSelecionado = PROJECT_TYPE_MAP[typeKey];

const ProjectIcon = projetoSelecionado?.icon || Code;
const projectLabel = projetoSelecionado?.label || "Não definido";
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Card className="border-muted bg-muted/10 p-3 sm:p-4 flex flex-col justify-between space-y-1">
          <CardHeader className="flex flex-row items-center gap-1.5 p-0 space-y-0">
            <div className="text-primary shrink-0">{<ProjectIcon size={14} />}</div>
          ...
          <CardContent className="p-0">
            ...
            <span className="text-[10px] sm:text-xs text-muted-foreground block mt-0.5 font-medium">{projectLabel}</span>
          </CardContent>
        </Card>
        ...
        <Card className="border-muted bg-muted/10 p-3 sm:p-4 flex flex-col justify-between space-y-0">
          ...
          <CardContent className="p-0 space-y-0.5 text-[11px] sm:text-xs">
            <div className="flex justify-between items-center gap-1">
              ...
              <span className="font-medium text-foreground truncate">{DELIVERY_ESTIMATE_MAP[data.project.deliveryEstimate] || "—"}</span>
            </div>
            <div className="flex justify-between items-center gap-1">
              <span className="text-muted-foreground">Verba:</span>
              <span className="font-semibold text-green-600 dark:text-green-400 truncate">{BUDGET_ESTIMATE_MAP[data.project.budgetEstimate] || "—"}</span>
            </div>
          </CardContent>
        </Card>
      </div>
